      ...
      <el-tab-pane label="地区管理" name="first" class="education-pane">
        <div class="search-box">
          ...
          <div class="select-exam">
            ...
            <el-button type="primary" class="mr-3 h-[2rem]" @click="searchLocation()" :loading="data.searchLocationLoading">搜索</el-button>
        ...
        <div class="location-list" v-loading="data.locationListLoading">
          <el-empty v-if="data.locationList.length === 0 && !data.locationListLoading"  description="暂无地区信息"/>
          <el-table v-if="data.locationList.length !== 0" :data="data.locationList" border style="width: 100%" max-height="380">
            ...
            <el-table-column fixed="right" label="操作" min-width="60">
              <template #default="scope">
                ...
                <el-button link type="primary" size="small" @click="removeLocation(scope.row.id)" :loading="data.removeLocationLoading">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!-- 分页 -->
      ...
      <el-tab-pane label="教学单位" name="second">
        <div class="search-box">
          ...
          <div class="select-exam">
            ...
            <el-button type="primary" class="mr-3 h-[2rem]" @click="searchCollege()" :loading="data.searchCollegeLoading">搜索</el-button>
          ...
          <el-table v-if="data.collegeList.length !== 0" :data="data.collegeList" border style="width: 100%" max-height="380">
            ...
            <el-table-column fixed="right" label="操作" min-width="60">
              <template v-slot="scope">
                ...
                <el-button link type="primary" size="small" @click="removeCollege(scope.row.id)" :loading="data.removeCollegeLoading">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!-- 分页 -->
      ...
      <el-tab-pane label="班级管理" name="third">
        <div class="search-box">
          ...
          <div class="select-exam">
            ...
            <el-button type="primary" class="mr-3 h-[2rem]" @click="searchClass()" :loading="data.searchClassLoading">搜索</el-button>
  ...
  <el-dialog v-model="data.createLocationModalVisible" title="地区创建" width="400" center>
    <div class="education-dialog">
      <el-form :model="data.newLocationForm" class="w-[20rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button class="w-[20rem]" type="primary" @click="submitLocationCreate()" :loading="data.submitLocationCreateLoading">提交创建</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog v-model="data.modifyLocationModalVisible" title="地区修改" width="400" center>
    <div class="education-dialog">
      <el-form :model="data.locationForm" class="w-[20rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button class="w-[20rem]" type="primary" @click="submitLocationModify()" :loading="data.submitLocationModifyLoading">提交修改</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog v-model="data.createCollegeModalVisible" title="教学单位创建" width="400" center>
    <div class="education-dialog">
      <el-form :model="data.newCollegeForm" class="w-[20rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button type="primary" class="w-[20rem]" @click="submitCollegeCreate()" :loading="data.submitCollegeCreateLoading">提交创建</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog v-model="data.modifyCollegeModalVisible" title="教学单位修改" width="400" center>
    <div class="education-dialog">
      <el-form :model="data.collegeForm" class="w-[20rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button class="w-[20rem]" type="primary" @click="submitCollegeModify()" :loading="data.submitCollegeModifyLoading">提交修改</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog v-model="data.createClassModalVisible" title="班级创建" width="600" center>
    <div class="education-dialog">
      <el-form :model="data.newClassForm" class="w-[30rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button type="primary" class="w-[40rem]" @click="submitClassCreate()" :loading="data.submitClassCreateLoading">创建新班级</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
  ...
  <el-dialog v-model="data.modifyClassModalVisible" title="班级修改" width="600" center>
    <div class="education-dialog">
      <el-form :model="data.currentClass" label-width="auto" class="w-[30rem]">
        ...
        <!-- 注册按钮 -->
        <el-form-item>
          <el-button type="primary" class="w-[40rem]" @click="submitClassModify()" :loading="data.submitClassModifyLoading">修改班级信息</el-button>
        </el-form-item>
      </el-form>
    </div>
  </el-dialog>
    ...
    <div class="education-dialog">
      <div class="dialog-search-box">
        ...
        <div class="select-exam">
          ...
          <el-button type="primary" class="mr-3 h-[2rem]" @click="searchClassApply()" :loading="data.searchClassApplyLoading">搜索</el-button>
        </div>
      </div>
      <!-- 所有班级信息展示 -->
      <div class="dialog-list" v-loading="data.classApplyListLoading">
        <el-empty v-if="data.classApplyList.length === 0 && !data.classApplyListLoading" description="暂无班级申请信息"/>
        <el-table v-if="data.classApplyList.length !== 0" :data="data.classApplyList" border style="width: 100%" max-height="400">
          ...
          <el-table-column fixed="right" label="操作" min-width="120">
            <template v-slot="scope">
              <el-button v-if="scope.row.status === 1" link type="primary" size="small" @click="passClassApply(scope.row.id)" :loading="data.passClassApplyLoading">通过</el-button>
              <el-button v-if="scope.row.status === 1" link type="primary" size="small" @click="rejectClassApply(scope.row.id)" :loading="data.rejectClassApplyLoading">不通过</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 分页 -->
