...
const searchUserReport = async () => {
  ...
  const res = await getStudentReportList(
    Number(data.subcourseId),
...
const handleRemove = (uploadFile: any, uploadFiles: any) => {
  ...
  for (
    let i = 0, len = data.newCourseReportForm.files_info.length;
    i < len;
    i++
  ) {
...
const submitCourseReportCreate = async () => {
  // 输入内容校验，报告标题、内容不能为空
  if (
    !data.newCourseReportForm.title ||
    data.newCourseReportForm.title.trim() === ""
  ) {
    ElMessage.error("请输入报告标题");
    return;
  }
  if (
    !data.newCourseReportForm.content ||
    data.newCourseReportForm.content.trim() === ""
  ) {
...
<template>
  <el-row class="course-report-page">
      ...
      <div class="report-list">
        ...
        <div class="search-box">
          ...
          <el-button
            ...
            :loading="data.searchUserReportLoading"
            >搜索</el-button
          >
        </div>
        <!-- 报告展示 -->
        <div class="report-table-list" v-loading="data.searchUserReportLoading">
          <el-empty
            v-if="
              data.courseReportList.length === 0 &&
              !data.searchUserReportLoading
            "
            description="暂无班级报告信息"
          />
          <el-table
            v-else
            :data="data.courseReportList"
            border
            style="width: 100%"
          >
            <el-table-column
    ...
    <el-col :span="8" class="create-report">
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">上传单个文件，且文件大小不超过20MB</div>
        </template>
      </el-upload>
      <el-button
        type="primary"
        class="report-btn mr-3"
        @click="submitCourseReportCreate"
        :loading="data.submitCourseReportCreateLoading"
        >创建报告</el-button
      >
    </el-col>
  </el-row>
  ...
  <el-dialog
    v-model="data.reportDetailModalVisible"
    title="报告详情"
    width="600"
    center
  >
    <el-descriptions direction="vertical" :column="4" border>
      <el-descriptions-item label="班级名" :span="2">{{
        data.currentReportDetail.class_name
      }}</el-descriptions-item>
      <el-descriptions-item label="报告内容" :span="2">{{
        data.currentReportDetail.content
      }}</el-descriptions-item>
      <el-descriptions-item label="发表时间" :span="4">{{
        data.currentReportDetail.create_time
      }}</el-descriptions-item>
    </el-descriptions>
  </el-dialog>
</template>
